...
class APIserver {
  ...
  if (response.statusCode == 200) {
    ...
    // Lấy danh sách ngôn ngữ từ thông tin phim
    List<List<String>> languagesList = data.map((movieData) {
      return List<String>.from(movieData['spoken_languages'].map((lang) => lang['english_name']));
    }).toList();

    // Trả về danh sách các Movie đã được thêm tên thể loại và ngôn ngữ
    return data.asMap().map((index, movieData) {
      return MapEntry(index, Movie.fromMap(movieData, genresList[index], languagesList[index]));
    }).values.toList();
  }
...
Future<Movie> getMovieDetail(int id) async {
  ...
  if (response.statusCode == 200) {
    ...
    // In ngôn ngữ để kiểm tra
    log('Languages: $languages');

    // Trả về đối tượng Movie với danh sách thể loại và ngôn ngữ
